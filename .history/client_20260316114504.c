    ...
    while(1) {
        ...
        if (n > 0 && type == 'B') {
            ...
            char **blurredMap = receiveMap(args->sockfd, args->width, args->height, 
                                           args->x, args->y, &localRows, &localCols);
            if (blurredMap) {
                if (!exitGame) {  // ← controllo globale, semplice
                    *(args->effectiveRows) = localRows;
                    *(args->effectiveCols) = localCols;
                    system("clear");
                    printMapUI(blurredMap, localCols, localRows, *(args->x), *(args->y), 
                              "MAPPA AGGIORNATA (blurrata)");
                    printf("\n  Comando [W/A/S/D] | list | exit > ");
                    fflush(stdout);
                }
                freeMap(blurredMap, localRows); // consuma sempre, stampa solo se !exitGame
            }
        }
        pthread_mutex_unlock(&socketMutex);
...
int main(int argc, char* argv[]) {
    ...
    exitGame = 1; // segnala al thread che la partita è finita, per evitare di stampare mappe nuove
