...
void printWinnerWithPipe(char *winner) {
    ...
    if (pid == 0) {
        ...
        execlp("sh", "sh", "-c",
            "sort -t';' -k3,3nr -k2,2nr score.txt | head -n1 | awk -F';' '{print $1}'",
            NULL);
...
if (nReady == 0) {
    ...
    /* NON scrivere sulla wakeup_pipe qui! */
}
pthread_mutex_unlock(&lobbyMutex);
...
/* graceful close */
shutdown(d->user, SHUT_WR);
char drain[16];
recv(d->user, drain, sizeof(drain), 0);

/* solo DOPO aver servito tutti, segnala al main di chiudere */
pthread_mutex_lock(&lobbyMutex);
nReady--;  /* usa un secondo contatore per sapere chi è l'ultimo a finire cleanup */
int last = (nReady == -nClients); /* tutti hanno fatto cleanup */
pthread_mutex_unlock(&lobbyMutex);
...
/* aspetta che il client legga il risultato prima di chiudere */
shutdown(d->user, SHUT_WR);  /* segnala fine invio senza chiudere subito */
char drain[16];
recv(d->user, drain, sizeof(drain), 0); /* aspetta che il client chiuda */
    /* ----- CLEANUP ----- */
