...
int main(int argc, char* argv[]) {
    ...
    response[1] = '\0';
    do {
        printf("Scegli cosa fare\n0: REGISTRAZIONE, 1: LOGIN");
        scanf("%d", &action);
    } while (action < 0 || action > 1);
    switch(action) {
        case 0:
            send(sockfd, "R", 1, 0);
            printf("Inserisci il nome utente da registrare: ");
            int n = read(0, username, sizeof(username));
            username[n] = '\0';
            username[strcspn(username, "\n")] = '\0';
            send(sockfd, username, strlen(username), 0);
            printf("Inserisci la password: ");
            n = read(0, password, sizeof(password));
            password[n] = '\0';
            ...
            printf("Inserisci il nome utente per il login: ");
            int n = read(0, username, sizeof(username));
            username[n] = '\0';
            username[strcspn(username, "\n")] = '\0';
            ...
            if(strcmp(response, "N") == 0) {
                printf("Login fallito: utente non trovato dopo la registrazione.\n");
                close(sockfd);
                return 1;
            }
            break;
        case 1:
            ...
            read(0, username, sizeof(username), 0);
            send(sockfd, username, strlen(username), 0);
            recv(sockfd, response, 2, 0);
            if(strcmp(response, "N") == 0) {
                printf("Login fallito: utente non trovato.\n");
                close(sockfd);
                return 1;
            }
            break;
