...
void *silentWaitBlurredMap(void *arg) {
    ...
    while(1) {
        ...
        if(n>0 && type == 'M') {
            /* consumiamo 'M' dal socket */
            recv(args->sockfd, &type, sizeof(char), 0);

            /* segnala al main di uscire dal loop e sblocca la read() su stdin */
            pthread_mutex_lock(&endMutex);
            end = 1;
            pthread_mutex_unlock(&endMutex);
            pthread_mutex_unlock(&socketMutex);

            /* aspetta 'E' poi W/L - ora siamo gli unici a leggere dal socket */
            char e;
            recv(args->sockfd, &e, 1, 0);
            char wol;
            int nr = recv(args->sockfd, &wol, 1, 0);
            if(nr > 0 && wol == 'W') {
                printf("\n🎉 VITTORIA! 🎉\n");
            } else {
                printf("\n😞 SCONFITTA! 😞\n");
...
int main(int argc, char* argv[]) {
    ...
    while (!checkEnd()) {
        ...
        if(n > 0 && response[0] == 'M') {
            /* consumiamo M e aspettiamo E+W/L tenendo il mutex:
             * il thread non puo' toccare il socket nel frattempo */
            char tmp;
            recv(sockfd, &tmp, 1, 0);
            char e;
            recv(sockfd, &e, 1, 0);
            char wol;
            int nr = recv(sockfd, &wol, 1, 0);
            pthread_mutex_unlock(&socketMutex);
            if(nr > 0 && wol == 'W') {
                printf("\n🎉 VITTORIA! 🎉\n");
            } else {
        ...
        if(n > 0 && response[0] == 'E') {
            /* timeout: consumiamo E e aspettiamo W/L, tenendo il mutex */
            char tmp;
            recv(sockfd, &tmp, 1, 0);
            char wol;
            int nr = recv(sockfd, &wol, 1, 0);
