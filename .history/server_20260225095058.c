...
void insertUser(const char *username) {
    ...
    if (!newNode) {
        log_error("malloc in insertUser");
        return;
    }
    ...
    if (usersList.tail) {
        usersList.tail->next = newNode;
    } else {
        usersList.head = newNode;
    }
    usersList.tail = newNode;
...
void removeUser(const char *username) {
    ...
    struct userNode *curr = usersList.head;

    while (curr != NULL) {
        if (strcmp(curr->username, username) == 0) {
            if (prev == NULL)
                usersList.head = curr->next;   /* era la testa */
            else
                prev->next = curr->next;

            if (curr->next == NULL)
                usersList.tail = prev;         /* era la coda */
...
void *newUser(void *arg) {
    ...
    pthread_mutex_lock(&lobbyMutex);
    nClients--;
    snprintf(logmsg, sizeof(logmsg), "[%s@%s] CLEANUP: rimosso da usersList, nClients=%d", d->username, d->ip, nClients);
    log_event(logmsg);
    if (nClients == 0) {
