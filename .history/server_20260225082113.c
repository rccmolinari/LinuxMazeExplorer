...
int registration(data * d) {
    char password[256];
        int nusr = recv(d->user, username, sizeof(username) - 1, 0);
        username[nusr] = '\0';
        int npass = recv(d->user, password, sizeof(password) - 1, 0);
        password[npass] = '\0';
        if(nusr < 0 || npass < 0) {
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: errore ricezione dati", d->ip);
            log_event(logmsg);
            close(d->user);
            free(d);
            return -2;
        }
        fdUsers = open("users.txt", O_RDWR | O_CREAT, 0644);
        if(fdUsers < 0) {
        ...
        while(read(fdUsers, line, sizeof(line)) > 0) {
            ...
            if(strcmp(line, username) == 0) {
                snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: username '%s' gia' esistente", username, d->ip, username);
                log_event(logmsg);
                close(fdUsers);
                close(d->user);
                free(d);
    ...
    if(!trovato) {
        ...
        close(fdUsers);
        snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: nuovo utente registrato", username, d->ip);
        log_event(logmsg);
    }
}
...
void *newUser(void *arg) {
    ...
    if(type == "R") {
        ...
        if(authentication(d) < 0) {
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: autenticazione fallita", d->username, d->ip);
            log_event(logmsg);
            close(d->user);
            free(d);
            return NULL;
