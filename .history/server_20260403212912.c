...
void *newUser(void *arg) {
    ...
    pthread_mutex_lock(&lobbyMutex);
    nClients--;
    int localClients = nClients;
    int localReady   = nReady;
    int alreadyStarted = gameStarted;
    pthread_mutex_unlock(&lobbyMutex);
    
    snprintf(logmsg, sizeof(logmsg), "SERVER: nClients dopo disconnect = %d", localClients);
    log_event(logmsg);
    ...
    } else if (!alreadyStarted && localReady > 0 && localReady == localClients) {
        pthread_mutex_lock(&lobbyMutex);
        gameStarted = 1;
        pthread_mutex_unlock(&lobbyMutex);
