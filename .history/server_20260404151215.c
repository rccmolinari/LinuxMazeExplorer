...
void gaming(struct data *d) {
    ...
    char buffer[256];
    while (!isTimeUp()) {
        int n = send(d->user, "", 0, 0); /* test di connessione */
        if (n <= 0) {
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] GAME: connessione persa durante la partita, client disconnesso", d->username, d->ip);
            log_event(logmsg);
...
/* loop pre-auth: il client può interrogare quanti giocatori sono connessi */
    do {
        n = send(d->user, "", 0, 0); /* test di connessione */
        if (n < 0) {
            snprintf(logmsg, sizeof(logmsg),
                    "[%s] AUTH: connessione persa prima di ricevere il tipo, client disconnesso", d->ip);
            log_event(logmsg);
            error = 1;
            break;
        }
    ...
    if (!error) {
        ...
        n = send(d->user, "", 0, 0); /* test di connessione prima di aspettare la chiusura */
        if (n < 0) {
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: connessione persa durante attesa chiusura", d->ip, d->username);
            log_event(logmsg);
            error = 1;
        }
        recv(d->user, &closeM, 1, 0); /* attende che il client chiuda la connessione */
    }
