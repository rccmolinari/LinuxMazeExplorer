...
int registration(struct data * d) {
    char password[256];
        ...
        if(fdUsers < 0) {
            log_error("open users.txt in newUser");
            close(d->user);
            free(d);
            return -2;
        }   
        char line[512];
        int trovato = 0;
        while(read(fdUsers, line, sizeof(line)) > 0) {
            strtok(line, " ");
...
void *newUser(void *arg) {
    ...
    if(n <= 0) {
        ...
        close(d->user);
        free(d);
        send(d->user, "N", 1, 0); /* N = No data received */
        return NULL;
    }
    if(type == "R") {
        int res = registration(d);
        if(res == -2) {
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: registrazione fallita", d->username, d->ip);
            log_event(logmsg);
            close(d->user);
            free(d);
            send(d->user, "N", 1, 0); /* N = No data received */
            ...
            log_event(logmsg);
            send(d->user, "Y", 1, 0); /* Y = Registration successful */
        }
        res = authenticate(d);
        if(res == -2) {
            ...
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: utente non registrato", d->username, d->ip);
            log_event(logmsg);
            close(d->user);
            free(d);
            send(d->user, "N", 1, 0); /* N = No data received */
            ...
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: autenticazione avvenuta con successo dopo registrazione", d->username, d->ip);
            log_event(logmsg);
            send(d->user, "Y", 1, 0); /* R = Registration successful */
        }
} else if(type == "L") {
        ...
        if(res == -2) {
            ...
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: utente non registrato durante login", d->username, d->ip);
            log_event(logmsg);
            close(d->user);
            free(d);
            send(d->user, "N", 1, 0); /* N = No data received */
            ...
            snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: autenticazione avvenuta con successo durante login", d->username, d->ip);
            log_event(logmsg);
            send(d->user, "Y", 1, 0); /* Y = Login successful */
        }
    }
