...
void insertUser(const char *username) {
    ...
    if (userList.tail) {
        ...
        userList.head = userList.tail = newNode;
    }
}
...
void sendUsers(int sockfd) {
    struct userNode *current = userList.head;
    while (current) {
        int len = strlen(current->username);
        send(sockfd, &len, sizeof(len), 0);
        send(sockfd, current->username, len, 0);
        current = current->next;
    }
}
/* --------------------------------------------------------------------------
 ...
 * -------------------------------------------------------------------------- */
int authenticate(struct data *d, char *usernamesrc) {
    char username[256];
    ...
    int fdopen_fd = open("users.txt", O_RDONLY);
    if (fdopen_fd < 0) {
    ...
    FILE *f = fdopen(fdopen_fd, "r");
    if (!f) {
        log_error("fdopen users.txt in authenticate");
        close(fdopen_fd);
        send(d->user, "NF", 2, 0);
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        line[strcspn(line, "\r\n")] = 0;
        /* il formato è "username password", confronta solo la prima parola */
    ...
    if (!found) {
        char logmsg[512];
        snprintf(logmsg, sizeof(logmsg), "[%s] AUTH: username '%s' non trovato", d->ip, username);
        log_event(logmsg);
        send(d->user, "NF", 2, 0);
        return 0;
    }
...
int registration(struct data *d) {
    ...
    if (readedbyte <= 0) {
        char logmsg[512];
        snprintf(logmsg, sizeof(logmsg), "[%s] AUTH: ricezione username fallita, client disconnesso", d->ip);
        send(d->user, "NF", 2, 0); /* NF = Not Found */
        log_event(logmsg);
        return 0;
    }
    
    int readedbyte2 = recv(d->user, passwd, sizeof(passwd) - 1, 0);
    if (readedbyte2 <= 0) {
        char logmsg[512];
        snprintf(logmsg, sizeof(logmsg), "[%s] AUTH: ricezione password fallita, client disconnesso", d->ip);
        send(d->user, "NF", 2, 0); /* NF = Not Found */
        log_event(logmsg);
        return 0;
    }
    ...
    strncpy(d->username, username, sizeof(d->username) - 1);
    d->username[sizeof(d->username) - 1] = '\0';
    passwd[readedbyte2] = '\0';
    passwd[strcspn(passwd, "\r\n")] = 0;
    int filefd = open("users.txt", O_RDONLY | O_CREAT | O_APPEND, 0644);
    //se utente già esiste, errore
    char line[512];
    int found = 0;
    if(filefd >= 0) {
    while (read(filefd, line, sizeof(line) - 1) > 0) {
    ...
    if (found) {
        char logmsg[512];
        snprintf(logmsg, sizeof(logmsg), "[%s] AUTH: utente già esistente, registrazione fallita", d->ip);
        log_event(logmsg);
        send(d->user, "NF", 2, 0); /* NF = Not Found */
        return 0;
    }
    ...
    filefd = open("users.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (filefd >= 0) {
        char buffer[512];
        int len = snprintf(buffer, sizeof(buffer), "%s %s\n", username, passwd);
        write(filefd, buffer, len);
        close(filefd);
    } else {
        log_error("[REGISTRATION] open users.txt");
        return 0;
    }
    send(d->user, "OK", 2, 0); /* OK = registrazione avvenuta con successo */
    char logmsg[512];
    snprintf(logmsg, sizeof(logmsg), "[%s@%s] AUTH: utente registrato", username, d->ip);
    log_event(logmsg);
    return 1;
}
...
int gaming(struct data *d) {
    ...
    while (!isTimeUp()) {
        ...
        if(!strcmp(buffer, "users")) {
            pthread_mutex_lock(&(d->socketWriteMutex));
            send(d->user, &nClients, sizeof(nClients), 0);
            sendUsers(d->user);
            sendAdjacentMap(d->user, d->map, d->width, d->height, d->x, d->y);
            pthread_mutex_unlock(&(d->socketWriteMutex));
            continue;
...
void *newUser(void *arg) {
    ...
    char logmsg[512];
    int choice = -1;
    int readedbyte = recv(d->user, &choice, sizeof(choice), 0);
        if(readedbyte <= 0) {
            snprintf(logmsg, sizeof(logmsg), "[%s] AUTH: ricezione scelta fallita, client disconnesso", d->ip);
            log_event(logmsg);
            pthread_mutex_lock(&lobbyMutex);
            nClients--;
            if(nClients == 0) write(wakeup_pipe[1], "X", 1);
            pthread_mutex_unlock(&lobbyMutex);
            close(d->user);
            free(d);
            return NULL;
            ...
            snprintf(logmsg, sizeof(logmsg), "[%s] AUTH: scelta non valida, client disconnesso", d->ip);
            log_event(logmsg);
            pthread_mutex_lock(&lobbyMutex);
            nClients--;
            if(nClients == 0) write(wakeup_pipe[1], "X", 1);
...
if (strcmp(gWinner, username) == 0) {
    ...
    nClients--;
    if(nClients == 0) {
        log_event("CLEANUP: ultimo client, segnale chiusura inviato al server");
