...
void sendUserList(struct data * d) {
    /* invia il numero di utenti in network byte order */
    int nUsersNet = htonl(userList.nUsers);
    send(d->user, &nUsersNet, sizeof(nUsersNet), 0);

    struct userNode *current = userList.head;
    while (current) {
        /* invia la lunghezza dello username (1 byte) seguita dallo username */
        uint8_t len = (uint8_t)strlen(current->username);
