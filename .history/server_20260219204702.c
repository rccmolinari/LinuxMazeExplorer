...
pthread_mutex_t winnerMutex = PTHREAD_MUTEX_INITIALIZER;
...
void *newUser(void *arg) {
    ...
    pthread_mutex_lock(&lobbyMutex);
    if (nExit == nClients) {
        // calcola vincitore solo l'ultimo thread
        printWinnerWithPipe(winner);
    
        pthread_mutex_lock(&winnerMutex);
        winnerCalculated = 1;
        pthread_cond_broadcast(&winnerCond);
    ...
    // tutti gli altri aspettano qui
    pthread_mutex_lock(&winnerMutex);
    while (!winnerCalculated)
