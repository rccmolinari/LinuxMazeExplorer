...
int registration(struct data *d) {
    /* ricevi "username;password" in un unico messaggio */
    char buf[512];
    int n = recv(d->user, buf, sizeof(buf) - 1, 0);
    if (n <= 0) return -2;
    buf[n] = '\0';
    buf[strcspn(buf, "\r\n")] = 0;

    /* separa username e password sul separatore ';' */
    char *sep = strchr(buf, ';');
    if (!sep) return -2;
    *sep = '\0';
    char *uname = buf;
    ...
    if (strlen(uname) == 0 || strlen(pass) == 0) return -2;

    strncpy(d->username, uname, sizeof(d->username) - 1);
    d->username[sizeof(d->username) - 1] = '\0';

    /* controlla se l'utente esiste gia' leggendo riga per riga */
    ...
    FILE *f = fdopen(fdUsers, "r+");
    if (!f) {
        log_error("fdopen users.txt in registration");
        close(fdUsers);
        return -2;
    }
    ...
    /* aggiunge la nuova riga "username;password\n" */
    fprintf(f, "%s;%s\n", d->username, pass);
    fclose(f);
    return 0;
}
 ...
 * authenticate
 *
 * Riceve "username;password" dal client, verifica che la coppia esista
 * in users.txt e popola d->username.
 * -------------------------------------------------------------------------- */
int authenticate(struct data *d) {
    ...
    if (n <= 0) {
        ...
        snprintf(logmsg, sizeof(logmsg),
                 "[%s] AUTH: ricezione credenziali fallita, client disconnesso", d->ip);
        log_event(logmsg);
        return -2;
    }
    buf[n] = '\0';
    ...
    *sep = '\0';
    ...
    /* cerca la coppia username;password in users.txt */
    FILE *f = fopen("users.txt", "r");
    if (!f) {
        log_error("fopen users.txt in authenticate");
        return -2;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *s = '\0';
        char *fileUser = line;
        char *filePass = s + 1;
        if (strcmp(fileUser, uname) == 0 && strcmp(filePass, pass) == 0) {
            fclose(f);
            return 0; /* credenziali corrette */
        }
    }
    fclose(f);
    return -1; /* utente non trovato o password errata */
}
